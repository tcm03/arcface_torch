...
def get_dataloader(
    ...
    local_rank,
    batch_size,
    dali=False,  # DALI is no longer needed, keep as False
    dali_aug=False,  # DALI augmentation is also not needed
    seed=2048,
    num_workers=2,
    ) -> Iterable:

    # Setup transforms for CelebA
    transform = transforms.Compose([
        transforms.RandomHorizontalFlip(),
        transforms.Resize((112, 112)),  # Ensure images are resized to the correct dimensions
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
    ])

    # Load CelebA dataset
    train_set = CelebA(root=root_dir, split='train', download=False, transform=transform)
    ...
    # Use standard DataLoader with BackgroundGenerator if desired
    train_loader = DataLoaderX(
        local_rank=local_rank,
        dataset=train_set,
        batch_size=batch_size,
...
class DataLoaderX(DataLoader):
    ...
    def __init__(self, local_rank, **kwargs):
        super(DataLoaderX, self).__init__(**kwargs)
        # Simplify if not using CUDA streams
        # self.stream = torch.cuda.Stream(local_rank)  # Remove this line
        self.local_rank = local_rank
    ...
    def preload(self):
        ...
        if self.batch is None:
            return None
        # if running on CPU, no need for CUDA streams
        # with torch.cuda.stream(self.stream):  # Remove this line
        #     for k in range(len(self.batch)):  # Remove this line
        #         self.batch[k] = self.batch[k].to(device=self.local_rank, non_blocking=True)  # Remove this line

    def __next__(self):
        # torch.cuda.current_stream().wait_stream(self.stream)  # Remove this line if running on CPU
        batch = self.batch
        if batch is None:
            raise StopIteration
        self.preload()
